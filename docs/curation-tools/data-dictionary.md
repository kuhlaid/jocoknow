...
Another thing to note is our `README.json` can be comprised of not only data we know up front about the data, but also data that can be inferred from any analysis we run against the data. For example, we likely want to include basic statistical features in our `README.json` to describe the dataset total observations (N), and possibly more fine-grained observations for categorical values in our variables. We will use scripts to generate the observations of our data and can then automatically insert them into the `README.json` since we want this same functionality across our datasets.
...
The main purpose of our `README.json` file is to ensure we have described all the variables and features found within our `dataTable.csv`. We will provide Python scripts that read the variable names and data from the `dataTable.csv` file and compare it to what we have defined in our `README.json` file. Some features of our `README.json` can be auto-generated from our `dataTable.csv` file, in which case the properties should match up. Our validation will answer questions such as 'are all variables described in the `README.json`' and do the variable properties match the data in the `dataTable.csv`.
